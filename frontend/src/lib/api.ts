...
export const fetchSyncLogs = async (): Promise<{
  logs: Array<{ timestamp: string; level: string; message: string; dept?: string }>;
}> => {
...
export const addComment = async (ticketId: string, content: string, isPublic = false) => {
  const r = await api.post('/api/actions/comment', {
    ticket_id: ticketId,
    content,
    is_public: isPublic,
  });
...
export const assignTicket = async (ticketId: string, agentId: string) => {
  const r = await api.post('/api/actions/assign', {
    ticket_id: ticketId,
    agent_id: agentId,
  });
  return r.data;
...
export const escalateTicket = async (ticketId: string, note?: string) => {
  const r = await api.post('/api/actions/escalate', {
    ticket_id: ticketId,
    note,
  });
  return r.data;
