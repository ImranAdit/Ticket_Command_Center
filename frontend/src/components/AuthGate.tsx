...
export const AuthGate: React.FC<AuthGateProps> = ({ onLogin }) => {
    ...
    // Get remembered email from localStorage, default to imran@adit.com
    const [rememberedEmail] = useState(() => 
        localStorage.getItem('lastAditEmail') || 'imran@adit.com'
    );
    ...
    // Note: useGoogleLogin hook setup would go here if real OAuth was active.
    // Since we want a single button that "stunningly" handles both dev and real,
    // we'll trigger the appropriate flow on click.
    
    const handleMainAction = () => {
        if (loading) return;
        
        // If Google Client ID is configured, this would trigger real OAuth.
        // For now, we perform the "Direct Login" but with the stunning UI.
        validateAndLogin(rememberedEmail);
    };
            ...
            <div className="glass-card p-12 w-[440px] max-w-full relative z-10 flex flex-col items-center">
                ...
                <div className="mb-4 flex items-center justify-center">
                    <img 
                        ...
                        alt="Adit" 
                        className="h-[52px] object-contain drop-shadow-[0_0_8px_rgba(255,255,255,0.2)]"
                    />
                </div>
                ...
                <div className="w-full flex flex-col items-center">
                    ...
                    >
                        ...
                        {loading ? (
                            ...
                            <>
                                <div className="p-2.5 bg-white rounded-lg flex items-center justify-center shadow-lg transition-transform duration-300 group-hover:scale-110">
                    ...
                    {error && (
                        <div className="mt-6 text-[11px] text-crimson-red bg-crimson-red/10 border border-crimson-red/20 py-2.5 px-4 rounded-lg text-center animate-shake w-full">
                            {error}
                        </div>
                    )}
                    ...
                    <div className="text-[11px] text-text-faint flex items-center justify-center gap-2 mt-10">
                        ...
                        <span className="tracking-tight font-medium uppercase opacity-50">Enterprise Adit Security</span>
                    </div>
                </div>
            </div>
        </div>
